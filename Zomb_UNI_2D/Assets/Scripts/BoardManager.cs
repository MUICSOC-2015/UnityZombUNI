...
public class BoardManager : MonoBehaviour {
		...
		for (int x = 1; x < columns + 1; x++) 
		{
			for(int y = 1; y < rows + 1; y++)
			{
				...
				if(x == 1)
				{
					...
					GameObject instance = Instantiate(leftWall, new Vector3 (x , y , 0f), Quaternion.identity) as GameObject;
					instance.transform.SetParent(boardHolder);
				}
				if(x == columns)
				{
					GameObject rightWall = rightWallTiles;
					GameObject instance = Instantiate(rightWall, new Vector3 (x , y , 0f), Quaternion.identity) as GameObject;
					instance.transform.SetParent(boardHolder);
				}
				if(x != columns && x != 1 && y == rows)
				{
					GameObject mac = macTiles;
					GameObject instance = Instantiate(mac, new Vector3 (x , y , 0f), Quaternion.identity) as GameObject;
					instance.transform.SetParent(boardHolder);
				}
				...
				if( x != 1 && x != 10 && y == 1)
				{
					...
					GameObject instance = Instantiate(board, new Vector3 (x , y , 0f), Quaternion.identity) as GameObject;
					instance.transform.SetParent(boardHolder);
				}
	...
	public void SetupScene(int level)
	{
		...
		Instantiate (exit, new Vector3 ( 1, 9, 0f), Quaternion.identity);
	}
